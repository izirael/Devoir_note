                                ...
                                <tbody>
                                    <c:forEach items="${parametres}" var="p">
                                        <tr>
                                            ...
                                            <td><span style="font-family: monospace; font-size: 1.1rem;">${p.gap}</span>
                                            </td>
                                            <td style="display: flex; gap: 0.5rem;">
                                                <a href="/admin/parametres/edit/${p.id}" class="btn btn-outline"
                                                    style="padding: 0.4rem 0.8rem; font-size: 0.75rem; color: var(--accent);">Modifier</a>
                                                <a href="/admin/parametres/delete/${p.id}" class="btn btn-outline"
                    ...
                    <div class="glass-card">
                        ...
                        <form action="/admin/parametres/save" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select name="matiere.idMatiere" required>
                                    <c:forEach items="${matieres}" var="m">
                                        <option value="${m.idMatiere}" ${editParametre.matiere.idMatiere==m.idMatiere
                                            ? 'selected' : '' }>${m.nom}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="operateur.id" required>
                                    <c:forEach items="${operateurs}" var="op">
                                        <option value="${op.id}" ${editParametre.operateur.id==op.id ? 'selected' : ''
                                            }>${op.symbole}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="resolution.id" required>
                                    <c:forEach items="${resolutions}" var="res">
                                        <option value="${res.id}" ${editParametre.resolution.id==res.id ? 'selected'
                                            : '' }>${res.nom}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="number" name="gap" placeholder="Ex: 5" value="${editParametre.gap}"
                                    required>
                            </div>
                            <button type="submit" class="btn btn-primary" style="width: 100%;">${editParametre != null ?
                                'Mettre a jour' : 'Ajouter le Bareme'}</button>
